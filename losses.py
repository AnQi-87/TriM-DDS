...
class SupConLoss(nn.Module):
    ...
    def forward(self, *views, labels=None, mask=None):
        ...
        if len(valid_views) < 2:
            return torch.tensor(0.0, device=views[0].device if views[0] is not None else 'cpu')
        
        features = self.get_forward_features_from_multi_view(*valid_views)  # shape [bsz, n_views, ...].
    ...
    def get_forward_features_from_multi_view(self, *views):
        ...
        for idx in range(len(views)):
            assert views[0].shape == views[idx].shape, f"View shapes mismatch: {views[0].shape} vs {views[idx].shape}"
        features = []
        for idx in range(len(views)):
