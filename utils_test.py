...
class TestbedDataset(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        if hasattr(data, "img_idx") and self.img_root is not None:
            data.image = self.get_image(f"{self.img_root}/{data.img_idx.item()}.png")

        if self.use_3d and hasattr(data, "img_idx"):
            feat3d = self.get_3d_feature(data.img_idx.item())
            if feat3d is not None:
               
                data.pos_3d = feat3d[:, :3].float()
                data.z_3d = torch.argmax(feat3d[:, 3:47], dim=1).long()
                data.num_nodes_3d = torch.tensor([feat3d.shape[0]], dtype=torch.long)
            else:
                data.pos_3d = torch.zeros((1, 3)).float()
                data.z_3d = torch.zeros(1, dtype=torch.long)
                data.num_nodes_3d = torch.tensor([1], dtype=torch.long)
